...
<title>Huskies Hunt | View/Edit Checkpoint</title>
<style>
...
<div class="form-db">
	<form action="checkpoints" method="POST" onsubmit="return validateForm()">
	 <fieldset>
	    <legend>Checkpoint #${checkpoint.id }</legend>
	    <input type="hidden" name="checkpointId" value="${checkpoint.id }" >
	 	<label>Name<span>*</span></label>
	    <input id="name" type="text" name="checkpointInfo" value="${checkpoint.name }">   
	    <label>Longitude<span>*</span></label>
	    <input id="longitude" type="text" name="checkpointInfo" value="${checkpoint.longitude }">
	    <label>Latitude<span>*</span></label>
	    <input id="latitude" type="text" name="checkpointInfo" value="${checkpoint.latitude }">
	    <label>Path ID</label>
	    <input type="text" name="checkpointInfo" value="${checkpoint.pathId }">
	     <span class="help-block">The path this checkpoint belongs to</span>
	    <label>Order in Path</label>
	    <input type="text" name="checkpointInfo" value="${checkpoint.pointOrder }">
	    <span class="help-block">A numerical value, defining in what order this point belongs in it's path</span>
	    <label>Code</label>
	    <input type="text" name="checkpointInfo" value="${checkpoint.code }">
	     <span class="help-block">The code a player must enter to receive the challenge</span>
	    <label>Challenge</label>
	    <textarea  name="checkpointInfo" rows=10 cols=10>${checkpoint.challenge }</textarea>
  		<label>Challenge Answer</label>
	    <textarea  name="checkpointInfo" rows=10 cols=10>${checkpoint.challengeAnswer }</textarea>
	    <label>Clue</label>
	    <textarea  name="checkpointInfo" rows=10 cols=10>${checkpoint.clue }</textarea>
	      <br>
...
<a href="checkpoints"><button type="reset" class="btn btn-primary cancel">Cancel</button></a>
...
$(document).ready(function(){
	...
	$('.btn-danger').on("click", function(){
		$('a#delete_url').attr("href","checkpoints?delete=${checkpoint.id}" )
	});
