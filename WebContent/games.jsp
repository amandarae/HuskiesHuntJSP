...
<title>Huskies Hunt | Games</title>
</head>
<body>
  	...
  	<tbody>
	  	<c:forEach var="g" items="${games}">
	 	    <tr>
	 			...
	 			<td>${g.startDate}</td> 
				<td>${g.endDate}</td>
			<td class="column-details"><a href="<c:url value="games?id=${g.id}"/>">View/Edit Details</a></td>
			<td class="column-details"><a href="#deleteModal" role="button" data-toggle="modal" class="red delete-btn" id="${g.id}">Delete</a></td>
	    	</tr>
		</c:forEach>
    </tbody>
</table>

<a href="add?game"><button type="button" class="btn btn-primary table-align">Add A Game</button></a>
...
$(document).ready(function() {
	...
	$('.delete-btn').on("click", function(){
		var id = $(this).attr("id");
		$('a#delete_url').attr("href","games?delete=" + id )
	});
});
</script>
</body>
